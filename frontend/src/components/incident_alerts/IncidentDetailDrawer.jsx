...
export default function IncidentDetailDrawer({ incident, onClose, onDispatch }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white border-l border-[#E4E4DF] shadow-2xl z-50 p-8 flex flex-col justify-between overflow-y-auto">
      <div className="space-y-6">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          ...
          <button onClick={onClose} className="p-1 rounded-xl text-slate-400 hover:text-slate-900 hover:bg-slate-100">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Evidence Snapshot */}
        <div className="relative rounded-2xl overflow-hidden border border-slate-300 bg-slate-950 h-52 flex items-center justify-center">
          <div className="absolute inset-0 bg-slate-900 flex flex-col items-center justify-center p-4 text-center">
            <Camera className="w-8 h-8 text-white mb-2 opacity-75" />
        ...
        <div className="bg-[#F4F4F0] border border-[#E4E4DF] rounded-2xl p-4 space-y-2">
          <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Detection Diagnostics</h4>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 space-y-1">
          <span className="text-[11px] font-bold text-amber-800 uppercase">Action Protocol:</span>
      ...
      <div className="pt-6 border-t border-slate-100 space-y-3">
        <button
          ...
          className="w-full py-3 bg-[#18181B] hover:bg-black text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 shadow-md transition-all"
        >
          <Send className="w-4 h-4" /> Dispatch RPF Quick Response Guard
        </button>
        <button
          onClick={onClose}
          className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl border border-slate-300"
        >
          Close Panel
        </button>
      </div>
