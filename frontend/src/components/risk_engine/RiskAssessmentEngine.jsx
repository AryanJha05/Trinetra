...
import { Activity, ShieldAlert, Cpu, CheckCircle2, AlertTriangle, Sliders, Info, ArrowRight, HelpCircle } from 'lucide-react';
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white border border-[#E5E7EB] rounded-xl p-5 shadow-2xs">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-[#111827] font-heading tracking-tight mt-0.5">
            Risk Assessment & Transparency Engine
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Transparent multi-factor threat computation and factor weight decomposition.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2 text-xs font-semibold">
          <span className="bg-[#111827] text-white border border-[#111827] px-3.5 py-2 rounded-full shadow-2xs flex items-center gap-2">
            <Cpu className="w-4 h-4 text-emerald-400" /> MODEL: TRINETRA-XAI-v2.4
          </span>
        </div>
      </div>

      {/* Interactive Risk Formula Builder & Live Simulator Card */}
      <div className="bg-slate-900 text-white border border-[#E5E7EB] rounded-xl p-5 space-y-5 shadow-2xs relative">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5 font-medium">
              Adjust weight factors below to observe real-time threat score synthesis:
            </p>
          </div>

          <div className="flex items-center space-x-3 font-sans">
            <span className="text-xs text-slate-400 font-medium uppercase">Synthesized Score:</span>
            <div className={`px-3 py-1 rounded-full text-xs uppercase ${severity.color}`}>
              {calculatedRisk}% · {severity.label}
            </div>
          </div>
        </div>

        {/* Explainable Equation Row */}
        <div className="bg-slate-800/80 border border-slate-700/80 p-4 rounded-xl flex flex-col md:flex-row items-center justify-around gap-4 font-mono text-xs">
          ...
          <span className="text-slate-500 text-lg font-bold">=</span>
          <div className="text-center bg-slate-900 px-4 py-2 rounded-lg border border-slate-700">
          ...
          {xaiCases.map((cs) => (
            ...
            >
              <div className="space-y-3">
                ...
                <h4 className="text-base font-bold text-[#111827] font-heading leading-snug">{cs.title}</h4>

                {/* Factors List */}
                <div className="space-y-1.5 pt-2 border-t border-[#F1F5F9] text-xs">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block font-sans">Contributing Factors:</span>
                  {cs.factors.map((f, i) => (
                    <div key={i} className="flex items-center justify-between bg-slate-50 p-2 rounded-lg border border-[#E5E7EB] text-xs font-medium">
                      <span className="text-slate-700 truncate max-w-[170px]">{f.factor}</span>
                      <span className="text-[#111827] font-bold font-mono">{f.contrib}</span>
                    </div>
                  ))}
                </div>

                <div className="bg-slate-900 text-white p-3.5 rounded-xl text-xs space-y-1">
                  <span className="text-[9px] font-mono text-emerald-400 font-bold uppercase tracking-wider block">Recommended Protocol:</span>
                  <p className="text-slate-200 leading-snug font-sans font-medium">{cs.recommendation}</p>
                </div>
              </div>
